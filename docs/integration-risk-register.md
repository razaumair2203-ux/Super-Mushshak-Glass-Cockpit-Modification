...
This register summarises principal aircraft-integration risks evidenced in project records. It is a current portfolio engineering view; detailed programme safety documentation is outside the public release.
...
| **RSK-003 — cross-domain interaction** | radio PTT activity coincided with short-duration engine-indication fluctuation | capture condition, isolate interface path, OEM investigation, require verification before closure | detailed corrective-action closure record not published |
| **RSK-004 — high-dynamic attitude behaviour** | ADAHRS compare/attitude-recovery behaviour appeared during high-rate manoeuvre/spin-recovery testing | capture observation, obtain OEM functional explanation, assess in training/test context | characterised; detailed final acceptance record not published |
...
The public register stays at aircraft-integration level: electrical behaviour, sensing, avionics interfaces, configuration state, HMI and supportability. Probability/severity scoring, controlled wiring detail and proprietary implementation data are outside this public release.
